...
interface Codecs {
  h264: 'nvenc_h264' | 'h264_videotoolbox' | 'h264';
  h265: 'nvenc_hevc' | 'hevc_videotoolbox' | 'hevc';
}

export class CodecError extends Error {
  constructor(
    message: string,
    public readonly codecs: Partial<Codecs>,
  ) {
    super(message);
...
export const codecs = binaries.then(async (bins): Promise<Codecs> => {
  ...
  const codecs: Partial<Codecs> = {
    h264: codecStr.includes('nvenc_h264')
      ? 'nvenc_h264'
      : codecStr.includes('h264_videotoolbox')
        ? 'h264_videotoolbox'
        : 'h264',
    h265: codecStr.includes('nvenc_hevc')
      ? 'nvenc_hevc'
      : codecStr.includes('hevc_videotoolbox')
        ? 'hevc_videotoolbox'
        : 'hevc',
  };
